	...
	public class Node : Object {
		...
		public bool as_bool (bool fallback = false) {
			if (inner.is_of_type(VariantType.BOOLEAN))
				...
				var val = inner.get_string ()._strip().ascii_down();
		...
		public int as_int (int fallback = 0) {
			if (inner.is_of_type(VariantType.INT64))
				return (int)inner.get_int64();
			return fallback;
		}

		public int64 as_int64 (int64 fallback = 0) {
			if (inner.is_of_type(VariantType.INT64))
				return inner.get_int64();
			return fallback;
		}
